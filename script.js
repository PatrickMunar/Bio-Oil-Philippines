...
const main = () => {
  ...
  const updateCubes = (object, time, numblobs) => {
    ...
    for (let i = 0; i < numblobs; i++) {
      ...
      const ballx =
        Math.sin(
          i + 1.26 * time * i * 0.25 * (1.03 + 0.5 * Math.cos(0.21 * i))
        ) *
          0.27 +
        0.5
      const bally =
        0.25 * Math.sin(time * Math.cos(i * 0.253 - 0.12) * 0.2) + 0.5
      const ballz =
  ...
  const navHoverOn = () => {
    if (isModalOut == false) {
      for (let i = 0; i < logoBars.length; i++) {
        gsap.to(logoBars[i], {
          duration: navHoverAnimationDuration,
          width: "6.5em",
          x: 0,
          y: logoBarValues[i].y,
          ease: "back",
  ...
  document
    ...
    .addEventListener("mouseleave", navHoverOff)
  ...
  const scrollDestinations = [
    ...
    document.querySelector(".purchaseSection").getBoundingClientRect().top -
      (sizes.height - document.querySelector(".purchaseSection").clientHeight) /
        2,
    document.querySelector(".photoSection").getBoundingClientRect().top -
      (sizes.height - document.querySelector(".photoSection").clientHeight) +
  ...
  for (let i = 0; i < navChoices.length; i++) {
    ...
    navChoices[i].addEventListener("pointerenter", () => {
      ...
      for (let j = 0; j < navLetters[i].length; j++) {
        gsap.to(navLetters[i][j], {
          duration: 0.1,
          delay: navTextAnimationDelay * j,
          fontStyle: "italic",
          y: -4,
        })
      }
    ...
    for (let j = 0; j < navChoices.length; j++) {
      if (navClickedIndex != j) {
        for (let k = 0; k < navLetters[j].length; k++) {
          gsap.to(navLetters[j][k], {
            duration: 0.1,
            delay: navTextAnimationDelay * k,
            fontStyle: "normal",
            y: 0,
          })
        }
  ...
  document.addEventListener("scroll", () => {
    ...
    if (isNavClicked == false) {
      if (scrollY < scrollDestinations[0]) {
        ...
        scrollY >= scrollDestinations[0] &&
        scrollY < scrollDestinations[1]
      ) {
        changeNavChoice(0)
      } else if (
        scrollY >= scrollDestinations[1] &&
        scrollY < scrollDestinations[2]
      ) {
        changeNavChoice(1)
      } else if (
        scrollY >= scrollDestinations[2] &&
        scrollY < scrollDestinations[3]
      ) {
        changeNavChoice(2)
  ...
  const clickVersusPoint = (i) => {
    if (versusBodyStates[i] == 0) {
      ...
      gsap.fromTo(
        versusPointButtons[i],
        { scale: 0.95 },
        { duration: 0.1, scale: 0.85 }
      )
      gsap.fromTo(
        versusPointButtons[i],
        { rotateZ: "45deg" },
        { duration: 0.2, delay: 0.1, rotateZ: "0deg", scale: 1 }
      )
  ...
  ScrollTrigger.create({
    ...
    start: () =>
      document.querySelector(".versusSection").clientHeight * 0.25 + " center",
  ...
  for (let i = 0; i < followLinkOuters.length; i++) {
    ...
    followLinkOuters[i].addEventListener("pointerenter", () => {
      ...
      gsap.to(followLinkOuters[i], {
        duration: 0.2,
        x: 2,
        y: -2,
        boxShadow: "inset 1px -1px 5px 0 #0000001a, -3px 3px 3px 0 #0000003a",
      })
    })
    ...
    followLinkOuters[i].addEventListener("pointerleave", () => {
      ...
      gsap.to(followLinkOuters[i], {
        ...
        x: 0,
        y: 0,
        boxShadow: "inset 1px -1px 5px 0 #0000001a, -1px 1px 3px 0 #0000003a",
      })
    })
  }
      ...
      document.addEventListener("pointermove", (e) => {
        ...
        if (isModalOut == true) {
          ...
          gsap.to("#modalContentPhotoLavender", {
            duration: 0,
            x: mouse.x * 5 - 5,
            y: (-mouse.y * 2 * window.innerWidth) / sizes.height,
            rotateZ: -mouse.x * 2 + 45,
          })
          gsap.to("#modalContentPhotoChamomile", {
            duration: 0,
            x: mouse.x * 5,
            y: (-mouse.y * 5 * window.innerWidth) / sizes.height,
            rotateZ: -mouse.x * 5 + 45,
          })
          gsap.to("#modalContentPhotoCalendula", {
            duration: 0,
            x: mouse.x * 5 + 5,
            y: (-mouse.y * 2 * window.innerWidth) / sizes.height,
            rotateZ: -mouse.x * 2 + 135,
          })
          gsap.to("#modalContentPhotoRosemary", {
            duration: 0,
            x: mouse.x * 5,
            y: (-mouse.y * 5 * window.innerWidth) / sizes.height,
          ...
          if (scrollY < scrollDestinations[1]) {
            ...
            if (productEnterValue == 0) {
              gsap.to(productChoices[0], {
                duration: 0,
                rotateY: -mouse.x * 10,
                rotateX: -mouse.y * 4,
              })
              gsap.to(productImages[0], {
                duration: 0,
                rotateY: mouse.x * 10,
                rotateX: mouse.y * 4,
              })
            } else if (productEnterValue == 1) {
              gsap.to(productChoices[1], {
                duration: 0,
                rotateY: -mouse.x * 10,
                rotateX: -mouse.y * 4,
              })
              gsap.to(productImages[1], {
                duration: 0,
                rotateY: mouse.x * 10,
                rotateX: mouse.y * 4,
              })
            ...
            if (scrollOnVersus == true) {
              ...
              for (let i = 0; i < particleGroup.length; i++) {
                const distanceFromPointerSquared =
                  ...
                  (particleGroup[i].position.y +
                    (versusGroup.position.y - camera.position.y) -
                    pointer.y) **
                    2
          ...
          deltaXY =
            ((((mouse.x - prevX) * window.innerWidth) / sizes.height) ** 2 +
              (mouse.y - prevY) ** 2) **
            0.5
    ...
    for (let i = 0; i < cards.length; i++) {
      ...
      if (swipeIndex < 0) {
        gsap.to(cards[i], {
          ...
          zIndex: cards.length - swipeIndex,
          rotateY: 0,
          rotateZ: 2.5 * swipeIndex + (Math.random() - 0.5) * 180,
          filter: "brightness(" + 100 - 100 * swipeIndex + "%)",
          x:
            (window.innerWidth * swipeIndex * -(mouse.x - prevX) + 0) *
            swipeSpeedMiltiplier,
          y:
            sizes.height *
            swipeIndex *
            (mouse.y - prevY) *
        ...
        gsap.to(cards[i], {
          duration: dt * 0.5,
          delay: cardIndexArray[i] * dt * 0.5 + dt,
          opacity: 0,
          ease: "none",
        })
      ...
      if (cardIndexArray[i] == cards.length) {
        gsap.to(cards[i], {
          duration: dt,
          delay: cardIndexArray[i] * dt * 0.5 + dt * 0.55,
          opacity: 1,
        })
      }
  ...
  for (let i = 0; i < cursorChoices.length; i++) {
    cursorChoices[i].addEventListener("pointerenter", () => {
      gsap.to(".cursorFollower", {
        ...
        width: "6rem",
        height: "6rem",
        backgroundColor: "#eb5f2800",
        ease: "back",
      })
      gsap.to(cursorSVGs[cursorIndices[i]], {
        duration: 0.25,
        scale: 0.6,
        ease: "back",
      })
  ...
  const modalContentPlantPhotos = document.querySelectorAll(
    ".modalContentPlantPhoto"
  )
  ...
  for (let i = 0; i < modalContentPlantPhotos.length; i++) {
    modalContentPlantPhotos[i].addEventListener("pointerenter", () => {
      ...
      if (modalContentPlantPhotos[i].id == "modalContentPhotoLavender") {
        ...
        modalContentPlantPhotos[i].id == "modalContentPhotoChamomile"
      ) {
        gsap.to(cursorTexts[1], { duration: 0.25, opacity: 1, scale: 1 })
      } else if (
        modalContentPlantPhotos[i].id == "modalContentPhotoCalendula"
      ) {
  ...
  const modalChartCenters = document.querySelectorAll(
  ...
  const fillModalChart = (i) => {
    ...
    gsap.fromTo(
      modalCharts[i],
      { background: "conic-gradient(#eb5f28 0% 0%, #eb5f2800 0%)" },
      {
        duration: 1.5,
        background:
          "conic-gradient(#eb5f28 0% " + p + "%, #eb5f2800 " + p + "%)",
        ease: "Power1.easeOut",
      }
    )
    countUp(0, i)
  ...
  gsap.to(".purchaseLinks1", {
    duration: 0,
    opacity: 0,
    pointerEvents: "none",
    boxShadow: "inset -1px 1px 5px 0 #0000001a, 0px 0px 0px 0 #0000003a",
  })
  gsap.to(".purchaseLinks2", {
    duration: 0,
    opacity: 0,
    pointerEvents: "none",
    boxShadow: "inset -1px 1px 5px 0 #0000001a, 0px 0px 0px 0 #0000003a",
  })
  gsap.to(".purchaseLinks3", {
    duration: 0,
    opacity: 0,
    pointerEvents: "none",
    boxShadow: "inset -1px 1px 5px 0 #0000001a, 0px 0px 0px 0 #0000003a",
  ...
  ScrollTrigger.create({
    ...
    start: () =>
      document.querySelector(".purchaseSection").clientHeight * 0.25 +
      " bottom",
      ...
      if (isModalOut == true) {
        ...
        for (let i = 0; i < modalChartsContainers.length; i++) {
          if (affectedChartsState[i] == 1) {
            dy[i] =
              modalChartsContainers[i].getBoundingClientRect().top -
              window.innerHeight
  ...
  const scrollTriggerJS = () => {
    ...
    gsap.fromTo(
      ...
      { y: 0 },
      {
        scrollTrigger: {
          ...
          start: () =>
            document.querySelector(".productSection").clientHeight * 0 +
            " bottom",
          end: () =>
            document.querySelector(".productSection").clientHeight * 0.5 +
            " center",
          // toggleActions: "play none none reverse",
          // ,
          scrub: true,
    ...
    gsap.fromTo(
      "#skincareChoice",
      { transformOrigin: "bottom center", x: "-20rem", rotateZ: "-10deg" },
      {
        scrollTrigger: {
          trigger: ".productSection",
          start: () =>
    ...
    gsap.fromTo(
      ...
      { opacity: 1 },
      {
        scrollTrigger: {
          trigger: ".productSection",
          start: () =>
            document.querySelector(".productSection").clientHeight * 0.2 +
            " bottom",
    ...
    // Versus
    gsap.fromTo(
      "#versusImageParallaxDiv1",
      { y: 100 * 5 },
      {
        scrollTrigger: {
          trigger: ".versusSection",
          start: () =>
            document.querySelector(".versusSection").clientHeight * 0 +
            2 +
            " bottom",
          end: () =>
            document.querySelector(".versusSection").clientHeight * 1 + " top",
          // toggleActions: "play none none reverse",
          // ,
          scrub: true,
          // pin: true,
    ...
    gsap.fromTo(
      ...
      { y: "5rem" },
      {
        scrollTrigger: {
          trigger: ".history1",
          start: () =>
            document.querySelector(".history1").clientHeight * 0 + " bottom",
          end: () =>
            document.querySelector(".history1").clientHeight * 1 + " top",
          // toggleActions: "play none none reverse",
          // ,
          scrub: true,
          // pin: true,
    ...
    gsap.fromTo(
      ...
      { y: "10rem" },
      {
        scrollTrigger: {
          ...
          start: () =>
            document.querySelector(".historySection").clientHeight * 0 +
            " bottom",
          end: () =>
            document.querySelector(".historySection").clientHeight * 1 + " top",
          // toggleActions: "play none none reverse",
          // ,
          scrub: true,
          // pin: true,
    ...
    gsap.fromTo(
      ".historyBanner1",
      { x: -document.querySelector(".historyBanner1").clientWidth },
      {
        scrollTrigger: {
          trigger: ".history1",
          start: () =>
            ...
            document.querySelector(".history1").clientHeight * 1 + " bottom",
    ...
    gsap.fromTo(
      ...
      { scale: 0 },
      {
        scrollTrigger: {
          trigger: ".history2",
          start: () =>
            document.querySelector(".history2").clientHeight * 0 + " bottom",
          end: () =>
            document.querySelector(".history2").clientHeight * 1 + " bottom",
          // toggleActions: "play none none none",
          // ,
          scrub: 1,
          // pin: true,
    ...
    gsap.fromTo(
      ...
      { scale: 0 },
      {
        scrollTrigger: {
          trigger: ".history3",
          start: () =>
            document.querySelector(".history3").clientHeight * 0.5 + " bottom",
          end: () =>
            document.querySelector(".history3").clientHeight * 1.5 + " bottom",
          // toggleActions: "play none none none",
          // ,
          scrub: 1,
          // pin: true,
      ...
      gsap.fromTo(
        ...
        { y: 0, rotateZ: 0 },
        {
          scrollTrigger: {
            ...
            start: () =>
              document.querySelector(".blankSection").clientHeight * 0 +
              " bottom",
            end: () =>
              document.querySelector(".blankSection").clientHeight * 1 +
              " bottom",
            // toggleActions: "play none none reverse",
            // ,
            scrub: true,
    ...
    gsap.fromTo(
      ...
      { y: "35rem" },
      {
        scrollTrigger: {
          trigger: ".purchaseSection",
          start: () =>
            document.querySelector(".purchaseSection").clientHeight * 0 +
            " bottom",
          end: () =>
            document.querySelector(".purchaseSection").clientHeight * 1 +
            " top",
          // toggleActions: "play none none reverse"
          // ,
          scrub: true,
